...
var series = chart.addAreaSeries({
    topColor: 'rgba(20, 20, 20, 1)',
    bottomColor: 'rgba(255, 255, 255, 0.0)',
    lineColor: 'rgba(163, 163, 163, 0.8)',
    lineWidth: 3
});
...
function setLastBarText() {
    ...
    toolTip.innerHTML = '<div style="font-size: 22px; margin: 4px 0px; color: #999999">' + data[data.length - 1].value + '</div>' +
        '<div>' + dateStr + '</div>';
}
...
chart.subscribeCrosshairMove(function (param) {
    if (param === undefined || param.time === undefined || param.point.x < 0 || param.point.x > width || param.point.y < 0 || param.point.y > height) {
        ...
        toolTip.innerHTML = '<div style="font-size: 22px; margin: 4px 0px; color: #999999">' + (Math.round(price * 100) / 100).toFixed(2) + '</div>' + '<div>' + dateStr + '</div>';
    }
});
